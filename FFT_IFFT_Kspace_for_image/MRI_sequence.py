...
print(f"X_Gradient_step = {X_Gradient_step} , Y_Gradient_step = {Y_Gradient_step}")
##Apply Gradient for Y
for Y_rows_total_gradient in range(ROWS):

    matrix_after_Y_G = image_to_matrix.copy() ##take copy as every time use Y gradient apply every thing for to matrix after move to x-y plane
    print(f"gradient value before multi to each Row = { Y_rows_total_gradient * Y_Gradient_step}")
    for row in range(ROWS):
        angle = Y_rows_total_gradient * Y_Gradient_step * row
        print(f"gradient value iterate = {row}  after multi to each Row = {angle}")
    ...
    for X_cols_total_gradient in range(COLS):
        ...
        print(f"gradient value before multi to each Column = {X_cols_total_gradient * X_Gradient_step}")
        for col in range(COLS):
            angle = X_cols_total_gradient * X_Gradient_step * col
            print(f"gradient value iterate = {col}  after multi to each Row = {angle}")
            Rx = np.array([[math.cos(math.radians(angle)), -math.sin(math.radians(angle)), 0],
                           [math.sin(math.radians(angle)), math.cos(math.radians(angle)), 0],
                           [0, 0, 1]])
            for row in range(ROWS):
                print(f"row = {row} , col = {col}")
                matrix_after_Y_G_X_G[row, col] = np.dot(Rx, matrix_after_Y_G_X_G[row, col])
                print(f"matrix_in_xy[row,col] = {matrix_after_Y_G_X_G[row, col]}")
        ...
        k_space[Y_rows_total_gradient, X_cols_total_gradient] = complex(y_sum, x_sum)  # complex k_space np.sqrt(x_sum**2 + y_sum**2)
...
plt.subplot(152), plt.imshow(np.log(1+np.abs(k_space)), "gray"), plt.title("K space ")
plt.subplot(153), plt.imshow(np.log(1+np.abs(abs(img_c5))), "gray"), plt.title(" shift")
plt.subplot(154), plt.imshow(np.log(1+np.abs(abs(img_c3))), "gray"), plt.title("inverse shift")
plt.subplot(155), plt.imshow(abs(img_c4), "gray"), plt.title("reconstructed image")
#
plt.show()
