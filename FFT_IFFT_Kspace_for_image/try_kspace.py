...
def RF_pulse(theta_RF):
    ...
    for i in range(image_gray.shape[0]):
        for j in range(image_gray.shape[1]):
            ...
            Mo_flipped_xy_plane = np.round(np.dot(R, new_matrix_image[i,j]), 2)
...
for  phase_y in  Gy_phases:
    ...
    for row in range(rows):
        for col in range(cols):
            matrix_in_xy[row, col] = np.round(np.dot(Ry, matrix_in_xy[row, col]), 2)
    ...
    for phase_x in Gx_phases:
        ...
        for row in range(rows):
            for col in range(cols):
                matrix_in_xy[row, col] = np.round(np.dot(Rx, matrix_in_xy[row, col]), 2)
        ...
        k_space[row_Kspace,col_Kspace] = np.sqrt(sum[0] ** 2 + sum[1] ** 2 + sum[2] ** 2)
...
plt.subplot(141), plt.imshow(image_gray, "gray"), plt.title("Original Image")
plt.subplot(142), plt.imshow(k_space, "gray"), plt.title("k_space")
plt.subplot(143), plt.imshow(np.log(1+np.abs(img_c4)), "gray"), plt.title("Spectrum")
plt.subplot(144), plt.imshow(np.log(1+np.abs(img_c5)), "gray"), plt.title("Processed Image")
